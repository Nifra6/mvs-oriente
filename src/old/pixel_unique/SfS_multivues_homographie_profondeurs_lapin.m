	...
	for indice_z = 1:nb_profondeurs
		...
		if condition_image & masque_2(i_2,j_2)
			...
			if abs(denominateur_pq) > 0
				...
				erreurs_mvs(indice_z) = (1/(2*rayon_voisinage+1)^2) * sum(abs(I_1_voisinage - I_2_voisinage_mvs),'all');
				erreurs_sfs(indice_z) = (1/(2*rayon_voisinage+1)^2) * sum(abs(I_1_voisinage - I_2_voisinage),'all');
