...
rayon_voisinage	= 1;			% Voisinage à prendre en compte
	...
	if condition_image & masque_2(i_2,j_2)
		...
		if abs(denominateur_pq) > 0.001
			...
			if (affichage_log)
				...
				sum(diff,"all")
				disp("===== Les gradients")
				dx_I_1_voisinage = dx_I_1(i_1-rayon_voisinage:i_1+rayon_voisinage,j_1-rayon_voisinage:j_1+rayon_voisinage)
				dy_I_1_voisinage = dy_I_1(i_1-rayon_voisinage:i_1+rayon_voisinage,j_1-rayon_voisinage:j_1+rayon_voisinage)
				dx_I_2_voisinage = reshape(interp2(dx_I_2, j_2_voisinage(:), i_2_voisinage(:),'cubic'),2*rayon_voisinage+1,2*rayon_voisinage+1)'
				dy_I_2_voisinage = reshape(interp2(dy_I_2, j_2_voisinage(:), i_2_voisinage(:),'cubic'),2*rayon_voisinage+1,2*rayon_voisinage+1)'
			end
		...
		hold on;
		normale_theorique = reshape(N_1(i_1,j_1,:),3,1);
		i_arrow_th = i_1 + normale_theorique(2);
		j_arrow_th = j_1 + normale_theorique(1);
		i_arrow = i_1 + normale(2);
		j_arrow = j_1 + normale(1);
		X_th = [j_1 j_arrow_th];
		Y_th = [i_1 i_arrow_th];
		X = [j_1 j_arrow];
		Y = [i_1 i_arrow];
