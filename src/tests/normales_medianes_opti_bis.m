function normales = normales_medianes_opti_bis(p_estimes,q_estimes)
	...
	bruh = zeros(1,nb_paire_image);
	size(liste_normales)
	size(bruh)
	bruh_bis = transpose(1:nb_paire_image) / 100;
	size(bruh_bis)
	quiver3(bruh,bruh,bruh+bruh_bis,liste_normales(1,:),liste_normales(2,:),liste_normales(3,:));
