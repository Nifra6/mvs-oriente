...
for indice_pixel = 1:nb_pixels_utilises
	...
	if condition_image & masque_2(i_2,j_2)
		...
		if abs(denominateur_pq) > 0.001
			...
			scores(indice_pixel) = sum((I_1(i_1-rayon_voisinage:i_1+rayon_voisinage,j_1-rayon_voisinage:j_1+rayon_voisinage) - I_2_voisinage).^2,'all');
...
XYZ_test = shapeFromDmOrtho(Z_1/pixelSize, masque_1);
...
XYZ_true_2 = shapeFromDmOrtho(z_true_2, masque_1);
...
q = zeros(size(p));
p(find(masque_1)) = liste_p_estimes;
q(find(masque_1)) = liste_q_estimes;
