...
while (1)
	...
	for indice_z_tilde = 1:nb_profondeurs_testees
		...
		P_1_tilde = [j_1 - u_0; i_1 - v_0; z_tilde];
		P_2_tilde = R_2' * P_1_tilde;
		i_2_tilde = round(P_2_tilde(2) + v_0);
		j_2_tilde = round(P_2_tilde(1) + u_0);
		...
		if condition_image_2 & masque_2(i_2_tilde,j_2_tilde)
			...
			if abs(coefficient_p_q) > 0
				...
				%if (indice_pixel == 12475 && z_tilde == round(Z_1(i_1,j_1)))
				if (z_tilde == round(Z_1(i_1,j_1)))
					z_1_decales
					z_tilde
					reshape(i_2_voisinage, 2*range + 1, 2*range + 1) - i_2_tilde
					reshape(j_2_voisinage, 2*range + 1, 2*range + 1) - j_2_tilde
					figure;
					subplot(2,2,1);
					...
					plot(i_2_tilde,j_2_tilde, 'r+', 'MarkerSize', 30, 'LineWidth', 2)
					hold off;
				end
